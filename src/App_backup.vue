      ...
      >
        <el-tab-pane label="添加单词" name="add">
          ...
          <div class="tab-content">
            <AddWord @word-added="refreshStats" v-if="currentTab === 'add'" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="查看记录" name="view">
          ...
          <div class="tab-content">
            <ViewWords @data-updated="refreshStats" v-if="currentTab === 'view'" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="搜索单词" name="search">
          ...
          <div class="tab-content">
            <SearchWords v-if="currentTab === 'search'" />
          </div>
        </el-tab-pane>
        <el-tab-pane label="单词练习" name="practice">
          ...
          <div class="tab-content">
            <PracticeWords v-if="currentTab === 'practice'" />
          </div>
        </el-tab-pane>
...
export default {
  ...
  setup() {
    ...
    const showNotification = (message, type = 'success') => {
      // 使用统一通知服务实现弹窗提示
      NotificationService.notify(message, getNotificationTitle.value, type)
    }
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (success) {
            ...
            // 计算日记数量
            if (importFormat.diaries) {
              diariesImported = Object.keys(importFormat.diaries).length
            }
            
            if (diariesImported > 0) {
...
.app-header-card {
  ...
  animation: statSlideIn 0.8s cubic-bezier(0.22, 1, 0.36, 1);
}
...
.stat-item::before {
  ...
  right: 0;
  height: 4px;
  background: linear-gradient(90deg, #3f37c9, #4895ef);
  transform: scaleX(0);
...
.tab-container {
  ...
  transition: all 0.3s ease;
  animation: statSlideIn 0.6s cubic-bezier(0.22, 1, 0.36, 1) 0.3s both;
...
.desktop-bottom-actions {
  margin-top: 2rem;
  text-align: center;
  animation: statSlideIn 0.6s cubic-bezier(0.22, 1, 0.36, 1) 0.5s both;
}
